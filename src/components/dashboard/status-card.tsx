...
export function StatusCard({ label, value, hint, tone = 'slate' }: { label: string; value: string; hint: string; tone?: keyof typeof toneStyles }) {
  return (
    <article className="rounded-[28px] border border-slate-200/80 bg-white p-5 shadow-[0_8px_30px_rgba(15,23,42,0.05)]">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={cn('inline-flex rounded-full px-3 py-1 text-xs font-semibold', toneStyles[tone])}>{hint}</span>
      </div>
    </article>
  );
}
