-- supabase/migrations/20260721161000_create_final_reset_iot_demo_data_rpc.sql
...
begin
  ...
  -- Prevent globally unique external keys from being reused by another home.
  if exists (
    select 1
    from public.devices as device
    where device.external_key in (
      'presence_main',
      'kitchen_gas_stove',
      'kitchen_light',
      'front_door_lock',
      ...
      and device.home_id <> p_home_id
  ) then
    raise exception using
      errcode = '23505',
      message = 'A demo external_key is assigned to another home.';
  end if;
  ...
  -- Shared reference data: upsert, never delete.
  insert into public.device_types (id, label, icon)
  values
    ('presence', 'Presence sensor', 'user-round-check'),
    ('switch', 'Switchable device', 'power'),
    ('lock', 'Door lock', 'lock-keyhole'),
    ('item', 'Item tracker', 'key-round')
  on conflict (id) do update
  set
    label = excluded.label,
    icon = excluded.icon;

  insert into public.device_type_states (
    device_type_id,
    state_key,
    label,
    is_safe_state
  ...
  set
    label = excluded.label,
    is_safe_state = excluded.is_safe_state;
  ...
  select
    ...
    jsonb_build_object(
      'simulated', true,
      'mqtt_topic', 'home/' || demo.external_key || '/state'
    )
  from (
    values
  ...
  join public.rooms as room
    on room.home_id = p_home_id
   and room.code = demo.room_code
  join public.device_type_states as safe_state
    on safe_state.device_type_id = demo.device_type_id
   and safe_state.state_key = demo.safe_state_key;
  ...
  select
    device.id,
    device.safe_device_type_state_id,
    'node-red-reset',
    now()
  from public.devices as device
  ...
  from (
    values
      ('kitchen_gas_stove', 'on',
        'Gas stove is still on. Turn it off.', 5::smallint),
      ('kitchen_light', 'on',
        'Kitchen light is still on.', 2::smallint),
      ('front_door_lock', 'unlocked',
        'Front door is unlocked. Lock it.', 4::smallint),
      ('house_keys', 'missing',
        'Do not forget your house keys.', 3::smallint)
  ) as rule(external_key, state_key, reminder_text, severity)
  join public.devices as device
    on device.home_id = p_home_id
   and device.external_key = rule.external_key
  join public.device_type_states as state
    on state.device_type_id = device.device_type_id
   and state.state_key = rule.state_key
...
revoke all on function public.reset_iot_demo_data(uuid, uuid, text)
from public;

grant execute on function public.reset_iot_demo_data(uuid, uuid, text)
to service_role;